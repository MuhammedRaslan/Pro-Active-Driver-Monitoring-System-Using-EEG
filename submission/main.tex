...
\ifx\revmode\@revclean
  \newcommand{\add}[1]{#1}                                   % additions -> plain text
  \newcommand{\del}[1]{}                                     % deletions -> removed entirely
\else
  \sethlcolor{addyellow}
  \newcommand{\add}[1]{\hl{#1}}
  \ifx\revmode\@revtrack
    \newcommand{\del}[1]{{\color{delred}\sout{#1}}}
  \else
    \newcommand{\del}[1]{}                                   % review mode: additions only
  \fi
\fi
\makeatother

% Replaced text: \chg{old}{new}
\newcommand{\chg}[2]{\del{#1}\add{#2}}
...
\thanks{\chg{A.~R.~Pal and G.~Murali Mohan are with the School of Mechanical Engineering, Vellore Institute of Technology (VIT) Chennai, Chennai 600127, Tamil Nadu, India (e-mail: abhishek.rudrapal@vit.ac.in; muralimohan.g@vit.ac.in). Corresponding author: A.~R.~Pal.}{A.~R.~Pal and G.~Murali Mohan are with the School of Mechanical Engineering, Vellore Institute of Technology (VIT) Chennai, Chennai 600127, Tamil Nadu, India (e-mail: abhishek.rudrapal@vit.ac.in; muralimohan.g@vit.ac.in).}}
...
\thanks{\add{Corresponding author: A.~Chemori.}}
\thanks{\add{ORCID: M.~R.~Thalassery, [TODO]; S.~S.~Ali, [TODO]; A.~R.~Pal, [TODO]; A.~Chemori, [TODO]; G.~Murali Mohan, [TODO].}}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{fig13_coherence_separation.png}
...
\begin{table}[t]
\caption{Awake-vs-drowsy \(O_1\)--\(O_2\) coherence by band on DROZY (\(n=7\,234\) awake, \(7\,264\) drowsy epochs). \(U\): Mann--Whitney; \(d\): Cohen's (drowsy\,$-$\,awake).}
\label{tab:coh}
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{l c c c c}
\toprule
Band & Awake (mean\(\pm\)sd) & Drowsy (mean\(\pm\)sd) & \(d\) & \(p\) \\
\midrule
\(\theta\) (4--8 Hz)   & \(0.564\pm0.144\) & \(0.528\pm0.149\) & \(-0.245\) & \(4.4\times10^{-53}\) \\
\(\alpha\) (8--13 Hz)  & \(0.469\pm0.128\) & \(0.435\pm0.132\) & \(-0.259\) & \(1.0\times10^{-51}\) \\
\(\beta\) (13--30 Hz)  & \(0.283\pm0.086\) & \(0.251\pm0.076\) & \(\mathbf{-0.403}\) & \(2.1\times10^{-126}\) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\columnwidth]{fig14_ema_raw_vs_smoothed.png}
...
\begin{table}[t]
\caption{Pro-active operating envelope on SEED-VIG (21 subjects). Behavioural onset is PERCLOS \(>0.70\) sustained 60~s. \add{\(\eta\) denotes the alarm threshold applied to the smoothed posterior; it is written \(\eta\) rather than \(\theta\) to avoid collision with the \(\theta\) frequency band (4--8~Hz).}}
\label{tab:proactive}
\centering
\small
\setlength{\tabcolsep}{3pt}
...
M.~R.~Thalassery designed the pipeline, implemented the experiments, conducted the ablations and statistical analyses, and drafted the manuscript. S.~S.~Ali contributed to the feature-extraction implementation and the live demonstrator. A.~R.~Pal supervised the project, provided guidance on signal-processing methodology, and reviewed and approved the final manuscript. \add{A.~Chemori [TODO: CRediT contribution to be supplied by A.~Chemori].} G.~Murali Mohan provided project supervision and reviewed the final manuscript.

\section*{Conflicts of Interest}
The authors declare no conflicts of interest. \chg{No external funding was received for this work.}{[TODO: confirm with A.~Chemori whether CNRS/LIRMM requires a funding or institutional-support acknowledgement; the statement ``no external funding was received for this work'' was accurate while the work was VIT-internal.]}
